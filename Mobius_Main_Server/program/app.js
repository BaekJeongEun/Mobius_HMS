 ...
 * - use HC-SR501 PIR sensor with 'onoff' Node.js module
...
const pir = new Gpio(26, 'in', 'rising');   // GPIO26 (pin): pir digital output

pir.watch((err, value) => {
    // console.log('PIR sensor detected: ' + value)
    if (tas_state == 'upload') {
        for (var i = 0; i < upload_arr.length; i++) {
            if (upload_arr[i].ctname == 'pir') {
                var cin = {
                    ctname: upload_arr[i].ctname,
                    // Change the locale and timezone if needed
                    con : new Date().toLocaleString(('ko-KR', { timeZone: 'UTC' }))};
                con = new Date().toLocaleString(('ko-KR', { timeZone: 'UTC' }));
                console.log(con);
